...
final class OS private(val name: OS.Name, val architecture: OS.Architecture) {
  override def toString: String = s"$name-$architecture"
}

object OS {
  sealed abstract class Name private (val marker: String, val archiveMode: String) extends Product with Serializable
  object Name {
  ...
  sealed abstract class Architecture extends Product with Serializable // private (val classifier: String)
  object Architecture {
  ...
  lazy val resolved: Option[OS] = for {
    name <- Name.resolved
