...
object PostgresSizeUpdater extends StrictLogging {
  ...
  lazy val downloadAttributes = Await.result(Future.sequence(for {
    ...
    try {
      ...
      try {
        ...
        logger.debug(s"Retrieved attributes for $ver-${download.variant} on OS $os - size: ${nf.format(size)} - sha256: ${Hex.encodeHexString(sha256)}")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val oldBody = Try { FileUtils.readFileToString(file, UTF_8) } getOrElse ""

    val sb = new StringBuilder
    for (DownloadAttributes(ver, variant, os, size, sha256) <- downloadAttributes) {
      ((sb ++= ver.toString += '-') append variant += ';' ++= os.toString += ';') append size += ';' ++= Hex.encodeHexString(sha256) += '\n'
    }
